...
{
    pItem ptr = getCurDepthStackHead(table->stack);
    while (ptr)
    {
        ptr = ptr->nextSymbol;
    }
        ...
        else
        {
            ...
            if (item != nullptr)
            {
                retItem = newItem(table->stack->curStackDepth,
                                  newFieldList(newString(item->field->name), newType(ARRAY, idx->val, copyType(item->field->type))));
                deleteItem(item);
                item = nullptr;
            }
        }
...
void Args(pNode node, pFieldList funcArgInfo, int lineno)
{
    ...
    */
    ...
    { 
        ...
        {
            printf("In Args, exp:%d, arg: %d\n", exp->kind, funcArgInfo->type->kind);
